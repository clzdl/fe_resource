	...
	(function($) {
	    ...
	    $.fn.citys = function(parameter,getApi) {
	        ...
	        return this.each(function() {
	            ...
	            var _api = {};
	            var $this = $(this);
	            var $province = $this.find('select[lay-filter="'+options.provinceField+'"]'),
	                $city = $this.find('select[lay-filter="'+options.cityField+'"]'),
	                $area = $this.find('select[lay-filter="'+options.areaField+'"]');
	            $.ajax({
	                url:options.dataUrl,
	                type:'GET',
	                crossDomain: true,
	                ...
	                success:function(data){
	                    ...
	                    var format = {
	                        ...
	                        area:function(){
	                            ...
	                            if(!hasCity){
	                            	...
	                            	$area.siblings("div").each(function(){
	                    ...
	                    //事件绑定
	                    form.on('select('+options.provinceField+')', function(data){
	                    	options.province = data.value;
	                        options.city = 0;
	                        options.area = 0;
	                        updateData();
	                        format.city();
	                    ...
	                    form.on('select('+options.cityField+')', function(data){
	                    	options.city = data.value;
	                        options.area = 0;
	                        updateData();
	                        format.area();
	                        options.onChange(_api.getInfo());
	                    ...
	                    form.on('select('+options.areaField+')', function(data){
	                    	options.area = data.value;
	                        options.onChange(_api.getInfo());
	                        form.render('select');
	                    });
	                    //初始化
